...
class MainMenuWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.user_label.setStyleSheet("font-weight: 700; color: #70404e;")
        ...
        for button in (rules_button, results_button, switch_button, exit_button):
        ...
        for button in (start_button, editor_button, rules_button, results_button, switch_button, exit_button):
            button.setMinimumHeight(46)
            layout.addWidget(button)
        layout.addStretch(1)

        info = QLabel("Совет: сначала откройте редактор и добавьте хотя бы 6 фото-меток.")
        info.setWordWrap(True)
        info.setAlignment(Qt.AlignCenter)
        info.setStyleSheet("color: #7a5962; font-size: 12px;")
